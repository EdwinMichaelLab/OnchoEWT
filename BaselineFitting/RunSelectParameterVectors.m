...
while ( NumParam < numSamples ) % numSamples = either SIR_samples or NParamVecs, depending on quality of MfData
    ...
    elseif OverallMf_asInput % Overall Mf
        ...
        [kId1, kId2] = get_modelFits_Mf_OverallData(MfData,...
            mfPrevArray,demog,da,ageMthMax,SIR_samples);
        kId = kId2;
...
% the site
% save(sprintf('../IO/OUT/test/ParamVectors%s.mat',Site,int2str(estABRflag)),'bCulex',...
%     'demog','ABR','ParameterVectors','L3Values','ageMthMax',...
%     'mfPrevArray');


save(sprintf('../IO/OUT/test/ParamVectors%s1.mat',Site),'bCulex','demog','ABR','ParameterVectors','L3Values','ageMthMax','mfPrevArray');
