...
param_max = [MBR_max+0.25*MBR_max; 250; 400; 3; 10; 0.95; 0.4];
...
subplot(1,3,2)
plot(MBR_0(:,id0),'Color',[0.7 0.7 0.7]);
hold on;
plot(median(MBR_0(:,id0),2),'Color','k','LineWidth',2);
plot(prctile(MBR_0(:,id0),2.5,2),'--k','LineWidth',2);
plot(prctile(MBR_0(:,id0),97.5,2),'--k','LineWidth',2);
errorbar(mean(MBR_cont,2),CI_c,'ko','MarkerFaceColor','k','LineWidth',1.2);

xlabel('Month');
ylabel('MBR');
set(gca,'YLim',[0 1.1*max(max(MBR_0))],'XLim',[1,11],'xtick',1:11,...
 'xticklabel',{'May','Jun','Jul','Aug','Sep','Oct','Nov','Dec','Jan','Feb','Mar'},'FontSize',10);
title('Control Sites','FontSize',10);
% xtickangle(45);

subplot(1,3,3)

plot(MBR_1(:,id1),'Color',[255 153 153]./255);
hold on;
plot(median(MBR_1(:,id1),2),'Color','r','LineWidth',2);
plot(prctile(MBR_1(:,id1),2.5,2),'--r','LineWidth',2);
plot(prctile(MBR_1(:,id1),97.5,2),'--r','LineWidth',2);
...
set(gca,'YLim',[0 1.1*max(max(MBR_0))],'XLim',[1,11],'xtick',1:11,...
 'xticklabel',{'May','Jun','Jul','Aug','Sep','Oct','Nov','Dec','Jan','Feb','Mar'},'FontSize',10);
title('Intervention Sites','FontSize',10);
% xtickangle(45);

fig = gcf;
fig.PaperUnits = 'centimeters';
fig.PaperPosition = [0 0 17.8 7.5];
% print('Control_vs_Intv_Fits','-dtiff','-r500');
