...
X1=[];
G1=[];
threeCTr=["CTr:0.4-0.5","CTr:0.7-0.8","CTr:0.9-1"];
CTrstr=["p4_p5","p7_p8","p9_1"];
etastr="p8to1";
...
for iSites = 2%:length(Sites)
    ...
    IdStr = [ThStr,VCStr];
    i=4;
    load(sprintf('../IO/OUT/reff100/Intv%s_v%s_reff100_CTr_%s_AnnualMDAnewparam_ABRth_newtest_eta_%s.mat',char(Sites{iSites}),...
            int2str(i),CTrstr(3),etastr));
...
G=[G;repmat(string(char(threeCTr(1))),length(z_mf(find(z_mf>0))),1);repmat(string(char(threeCTr(2))),length(z2_mf(find(z2_mf>0))),1);repmat(string(char(threeCTr(3))),length(z3_mf(find(z3_mf>0))),1)];

G1=[G1;repmat(string(char(threeCTr(1))),length(z_ATP(find(z_ATP>0))),1);repmat(string(char(threeCTr(2))),length(z2_ATP(find(z2_ATP>0))),1);repmat(string(char(threeCTr(3))),length(z3_ATP(find(z3_ATP>0))),1)];
...
set(gca, 'XTickLabel',{threeCTr(1),threeCTr(2),threeCTr(3)},'Fontsize',18,'LineWidth',3)
