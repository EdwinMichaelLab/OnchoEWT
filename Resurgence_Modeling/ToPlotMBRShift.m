...
for iSites = 2%1:length(Sites)
    ...
    load(sprintf('../IO/OUT/Intv%s_%s_v4_TBR_AprStart_s1.mat',char(Sites{iSites}),IdStr),...
        'MBRIntv','slash_t');
    plot(median(MBRIntv(1:24,:),2),'r');
    
    load(sprintf('../IO/OUT/Intv%s_%s_v4_TBR_AprStart_s3.mat',char(Sites{iSites}),IdStr),...
        'MBRIntv','slash_t');
    plot(median(MBRIntv(1:24,:),2),'g');
    
    load(sprintf('../IO/OUT/Intv%s_%s_v4_TBR_AprStart_s6.mat',char(Sites{iSites}),IdStr),...
        'MBRIntv','slash_t');
    plot(median(MBRIntv(1:24,:),2),'k');
    
    z = gca;
    ymax = z.YLim(2);
    set(gca,'XLim',[1 24],'YLim',[0 ymax],'xtick',1:1:24,'xticklabel',...
...
%     load VC scenarios, VC = 1
    ...
    for i = [1,2,3]
        ...
        load(sprintf('../IO/OUT/Intv%s_%s_v%s_TBR_AprStart_s1.mat',char(Sites{iSites}),IdStr,int2str(i)),...
            'MBRIntv','slash_t');
        plot(median(MBRIntv(1:24,:),2),'r');
        
        load(sprintf('../IO/OUT/Intv%s_%s_v%s_TBR_AprStart_s3.mat',char(Sites{iSites}),IdStr,int2str(i)),...
            'MBRIntv','slash_t');
        plot(median(MBRIntv(1:24,:),2),'g');
        
        load(sprintf('../IO/OUT/Intv%s_%s_v%s_TBR_AprStart_s6.mat',char(Sites{iSites}),IdStr,int2str(i)),...
            'MBRIntv','slash_t');
        plot(median(MBRIntv(1:24,:),2),'k');
