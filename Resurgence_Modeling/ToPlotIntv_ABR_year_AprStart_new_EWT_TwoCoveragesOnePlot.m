...
%         'MBRIntv','slash_t');
    load(sprintf('../IO/OUT/Intv%s_v%s_reff200_CTr_p8_1_AnnualMDAnewparam_ABRth_newtest_eta_p9top95.mat',char(Sites{iSites}),...
          int2str(i)));
...
%      set(gca,'XLim',[1 24],'YLim',[0 ymax],'xtick',1:2:24,'FontSize',10);
    set(gca,'XLim',[1 60],'YLim',[0 ymax],'xtick',1:2:60,'xticklabel',...
    ...
    for i = [1,2,3]
        j = j+2;
        j1=j1+1;
        load(sprintf('../IO/OUT/Intv%s_v%s_reff200_CTr_p4_p6_AnnualMDAnewparam_ABRth_newtest_eta_p9top95.mat',char(Sites{iSites}),...
...
%         %'../IO/OUT/Intv%s_v%s_reff200_CTr_p8_1_biAnnualMDAnewparam_ABRth.mat'
   ...
   %plot(prctile(MBRIntv(1:30,:),97.5,2),'--','Color','b')
    ...
    plot(median(MBRIntv(1:30,:),2),'b');        tS = find(slash_t < 25);
        ...
        set(gca,'box','off')
        line([1 60],[TBR TBR],'LineStyle','--','Color','k'); % median TBR Masaloa
        set(gca,'XLim',[0.5 60]);
    z.LineWidth=2;            % set the axis linewidth for box/ticks
