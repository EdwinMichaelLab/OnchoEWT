...
for iSites = 2%1:length(Sites)
    ...
    load(sprintf('../IO/OUT/Intv%s_v%s_reff200_CTr_p8_1_AnnualMDA_oldtest.mat',char(Sites{iSites}),int2str(i)),...
        'MBRIntv','slash_t');
    %load(sprintf('../IO/OUT/Intv%s_v%s_reff200_CTr_p8_1_biAnnualMDA.mat',char(Sites{iSites}),...
    %       int2str(i)));
    subplot(4,1,1);
...
%      set(gca,'XLim',[1 24],'YLim',[0 ymax],'xtick',1:2:24,'FontSize',10);
    ...
    for i = [1,2,3]
        j = j+1;
        load(sprintf('../IO/OUT/Intv%s_v%s_reff200_CTr_p8_1_AnnualMDAnewparam.mat',char(Sites{iSites}),...
