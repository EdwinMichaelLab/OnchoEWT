function [mfPrevIntv,MBRIntv,L3Intv,pIntv,wIntv,MfInt,Eff,Lam,CTr] = RunOnchoInterventionScenarios_EWTraps(k,...
    ParameterVectors,L3Values,demoX,ageMthMax,da,bCulex,...
    AgeLimits,drugCoverage,VillageDrugEfficacy,...
    MDAInterval,NumYears,MultiVecMBR,VC0,slash_t,TBR,trapcovereagestr)%[mfPrevIntv,MBRIntv,L3Intv,pIntv,wIntv,MfInt,CTr]
...
%trapcovereagestr='p8_1';
% slash parameters
% MBR_max, R_L, R_U, k1, k2, n, A, b1, b2, b3, b4
load(strcat('MBRFunParams_Traps_ctr_',trapcovereagestr,'.mat'))
...
parfor i = 1:length(k)
    ...
    % treatments
    [VH,Eta,A,Cr] = Temporal_Change_in_VoverH_due_to_VC_EWTraps(VC0,...
