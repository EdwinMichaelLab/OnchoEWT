function [VH,n,A,Cr] = Temporal_Change_in_VoverH_due_to_VC_EWTraps(VC0,...
    NumYears,beta,P,R0,slash_t,MultiVecMBR)%[VH,Cr]
...
if VC0 == 1
    ...
    for t = 1:length(R) % time since slash
        ...
        if ts < 1
            ...
            MBR(t) = MBR_0(t)*(1-n*exp(-A*ts)*Cr);%(1-n*exp(-A*ts)*Cr); % Trap/slash efficacy function n*(1-exp(-A*ts));
        end
        if i < length(slash_t)
            if t == slash_t(i+1)
                i = i+1;
