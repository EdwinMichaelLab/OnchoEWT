...
load('../IO/IN/Inter_IN_biAnnualMDA.mat');%Inter_IN_AnnualMDA.mat
...
dAp=[20];%[0,20,30,40,50,60,70];
for strdA=1:length(dAp)
    dA=A*dAp(strdA)/100;
    trapcovereagestr={'p4_p6','p6_p8','p8_1'};
    ABRfactor=1/(1-dA/A);
    MultiVecMBR = ABRfactor*ABR/12;
    ...
    for st=1:numel(trapcovereagestr)
        trapstr = trapcovereagestr{st};
        ...
        for v = 1:3% 4 is used only when no vector control is simulated
            ...
            [mfPrevIntv,MBRIntv,L3Intv,pIntv,wIntv,MonthsMDA,MonthlyMDACov,MfInt,Eff,Lam,CTr] ...
                = Oncho_Intervention_MDA_VC(...
                ParameterVectors,L3Values,demog,ageMthMax,bCulex,...
                AgeLimits,MonthlyMDACov,RegimenEfficacy,...
                MDAInterval,NumYears,MultiVecMBR,VC,slash_t,EP_Th,TBR,trapstr);   
            
            %Ac=A*CTr;
            NN4=A*CTr/(pi*reff*reff);
            NN=A*CTr*(1-dA/A)/(pi*reff*reff);
            dd=(sqrt(A)-2*reff)./(sqrt(NN4)-1);
           % save(sprintf('../IO/OUT/Intv%s_v%s_reff200_CTr_p8_1_AnnualMDA.mat',char(Sites{iSites}),int2str(v)),...
       %biAnnualMDAnewparam_ABRth.mat
    %        save(sprintf('../IO/OUT/Intv%s_v%s_reff200_CTr_p8_1_AnnualMDAnewparam_ABRth.mat',char(Sites{iSites}),int2str(v)),...
    %             'mfPrevIntv','MBRIntv','L3Intv','pIntv','wIntv','RegimenEfficacy','MonthlyMDACov',...
    %             'MfData','EP_Th','MonthsMDA','slash_t','TBR','NN','dd','CTr');
           save(sprintf(strcat('../IO/OUT_ABRbumped/Intv%s_v%s_reff200_CTr_',trapstr,'_biAnnualMDAnewparam_ABRth_new_ABRfactor_','dA',num2str(dAp(strdA)),'p.mat'),char(Sites{iSites}),int2str(v)),...
                'mfPrevIntv','MBRIntv','L3Intv','pIntv','wIntv','RegimenEfficacy','MonthlyMDACov',...
                'MfData','EP_Th','MonthsMDA','slash_t','TBR','NN','dd','Eff','Lam','CTr');       
        end
